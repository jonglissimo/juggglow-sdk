        ...
        peripheral.discoverAllServicesAndCharacteristics(function(error, services, characteristics) {
          if (error) { reject(error); }
            ...
            if (juggglowService) {
              ...
              var batteryLevel = characteristics.filter(function(characteristic) {
                return characteristic.uuid === batteryCharacteristicUuid;
              })[0];

              var manufacture = characteristics.filter(function(characteristic) {
                return characteristic.uuid === manufactureUuid;
              })[0];

              var modelNo = characteristics.filter(function(characteristic) {
                return characteristic.uuid === modelNoUuid;
              })[0];

              var serialNo = characteristics.filter(function(characteristic) {
                return characteristic.uuid === serialNoUuid;
              })[0];

              var firmwareRev = characteristics.filter(function(characteristic) {
                return characteristic.uuid === firmwareRevUuid;
              })[0];

              var softwareRev = characteristics.filter(function(characteristic) {
                return characteristic.uuid === softwareRevUuid;
              })[0];

              var hardwareRev = characteristics.filter(function(characteristic) {
